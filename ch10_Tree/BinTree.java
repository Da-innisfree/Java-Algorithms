...
public class BinTree<K, V> {
    ...
    private void printReserveTree(Node node) {
        if (node != null) {
            printSubTree(node.right);       //오른쪽 서브 트리를 키 값의 내림차순으로 출력
            System.out.println(node.key + " " + node.data);     //node를 출력
            printSubTree(node.left);        //왼쪽 서브 트리를 키 값의 내림차순으로 출력
        }
    }

    //모든 노드를 키 값의 내림차순으로 출력
    public void printReserve() {
        printReserveTree(root);
    }
