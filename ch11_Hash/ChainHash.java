...
public class ChainHash<K, V> {
    ...
    public V search(K key) {
        ...
        while (p != null) {
            if (p.getKey().equals(key))
                return p.getValue();        //검색 성공 후 데이터 리턴
            p = p.next;     //다음 노드에 주목
        }
        return null;        //검색 실패
    }
    ...
    public int add(K key, V data) {
        ...
        Node<K,V> temp = new Node<>(key,data, table[hash]);     //기존 버킷의 연결리스트의 맨 앞에 추가
    ...
    public int remove(K key){
        ...
        while (p!=null){
            if(p.getKey().equals(key)){     //해당 노드를 찾으면
                if (pp==null)
                    table[hash]=p.next;
                else
                    pp.next = p.next;       //삭제할 노드 앞의 노드가 있는 경우
                return 0;
            }
            pp=p;           //이전 노드는 p를 가르킵니다.
            p=p.next;       //다음 노드로 변경합니다.
        }
